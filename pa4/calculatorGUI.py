...
# date: March 6, 2023
# file: calculaterGUI.py is a program that implements a calculater user interface
# input: button clicks
# output: float value 
...
def calculator(gui):   
    ...
    b_pt = addButton(gui,entrybox,'.')
    b_cp = addButton(gui,entrybox,')')
    b_pr = addButton(gui,entrybox,"^")

    # add buttons to the grid
    buttons =  [b_clr, b_pr, b_pt, b_div,
                b7,    b8,   b9,   b_mult,
                b4,    b5,   b6,   b_sub, 
                b1,    b2,   b3,   b_add, 
                b_op, b0, b_cp, b_eq,]
...
def clickButton(entrybox, value):
    ...
    if value == '=':
        try:
            #print(calculate(exp))
            exp = str(calculate(exp))
            updateScreen(entrybox,exp)
        except:
            updateScreen(entrybox,"Error")
    elif value == 'c':
        exp = ''
        updateScreen(entrybox,exp)
    else:
...
def updateScreen(entrybox,value):
